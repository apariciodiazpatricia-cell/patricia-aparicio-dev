...
export default function Footer() {
    ...
    const handleCopyEmail = () => {
        navigator.clipboard.writeText('apariciodiazpatricia@gmail.com');
        setCopied(true);
    ...
    return (
        <footer className="w-full relative pt-12 pb-16 px-6 md:px-16 overflow-hidden bg-black border-t border-[#222] mt-16">

            {/* Animación de deslizamiento de lado a lado para el droide */}
            <style>{`
                @keyframes patrolRobot {
                    0% { transform: translateX(0px); }
                    50% { transform: translateX(calc(100vw - 320px)); }
                    100% { transform: translateX(0px); }
                }
                .robot-patrol {
                    animation: patrolRobot 18s infinite ease-in-out;
                }
            `}</style>

            {/* Tarjeta contenedora principal extendida de lado a lado */}
            <div className="w-full relative z-10 bg-[#121212] border border-[#222] hover:border-[#00FF66]/40 transition duration-500 rounded-2xl p-8 md:p-12 shadow-[0_0_40px_rgba(0,0,0,0.6)]">

                {/* Franja de luz láser superior */}
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3/4 h-px bg-gradient-to-r from-transparent via-[#00FF66] to-transparent"></div>

                {/* Droide Patrullero Grande Integrado */}
                ...
                <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 items-center justify-between pt-4">
                    ...
                    {/* Identidad y Frase Real Directa */}
                    <div className="lg:col-span-5 space-y-3">
                        ...
                        <h3 className="text-xl font-extrabold text-white tracking-wide">
                            Patricia Aparicio <span className="text-[#00FF66]">// Fullstack Dev</span>
                        </h3>
                ...
                <div className="mt-10 pt-6 border-t border-[#222] flex flex-col sm:flex-row items-center justify-between text-xs font-mono text-[#777] gap-4">
                    <div>
                        © 2026 Patricia Aparicio •
                    </div>
                    <div className="flex gap-6">
                        <a href="https://github.com/apariciodiazpatricia-cell" target="_blank" rel="noreferrer" className="hover:text-[#00FF66] transition">GitHub</a>
                        <a href="https://www.linkedin.com/in/patriciaapariciodiaz/" target="_blank" rel="noreferrer" className="hover:text-[#00FF66] transition">LinkedIn</a>
                        <a href="mailto:apariciodiazpatricia@gmail.com" className="hover:text-[#00FF66] transition">Email</a>
                    </div>
                </div>
